...
def test_td3_basic_functionality():
    ...
    for i in range(policy_delay + 2):
        ...
        if (agent.total_it % policy_delay) == 0:
            assert metrics.get("policy_update", False) == True, f"Policy should update on iteration {agent.total_it}"
        else:
            assert metrics.get("policy_update", False) == False, f"Policy should NOT update on iteration {agent.total_it}"
