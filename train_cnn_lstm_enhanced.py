...
class EnhancedCNNLSTMTrainer:
    ...
    def __init__(
        ...
        self.model: CNNLSTMModel = CNNLSTMModel(input_dim=1, config=model_config)  # Placeholder, will be recreated
    ...
    def _train_epoch(
        ...
        for batch_idx, (data, target) in enumerate(pbar):
            ...
            grad_norm = torch.nn.utils.clip_grad_norm_(
                self.model.parameters(), max_norm=self.training_config.get("max_grad_norm", 1.0)
            )
    ...
    def plot_training_history(self, save_path: str | None = None) -> None:
        ...
        # Remove plt.show() to prevent blocking in non-interactive environments
...
class HyperparameterOptimizer:
    ...
    def objective(self, trial: optuna.Trial) -> float:
        ...
        if len(cnn_filters) != len(cnn_kernel_sizes):
            raise ValueError(
                f"CNN architecture mismatch: {len(cnn_filters)} filters vs {len(cnn_kernel_sizes)} kernels"
            )
        ...
        try:
            ...
            if isinstance(e, (ValueError, TypeError, AttributeError)):
                logger.exception(f"Critical configuration error: {e}")
                raise
            return float("inf")
