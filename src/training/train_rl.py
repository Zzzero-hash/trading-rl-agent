...
def train_from_config(config_path: str) -> None:
    """Load YAML config and run training."""
    import yaml
    with open(config_path, "r") as f:
        cfg = yaml.safe_load(f) or {}
    main_args = [
        "--data", cfg.get("data", "data/sample_data.csv"),
        "--model-path", cfg.get("model_path", "models/cnn_lstm_trained.pth"),
    ]
    if "num_workers" in cfg:
        main_args += ["--num-workers", str(cfg["num_workers"])]
    if "num_gpus" in cfg:
        main_args += ["--num-gpus", str(cfg["num_gpus"])]
    if cfg.get("cluster_config"):
        main_args += ["--cluster-config", cfg["cluster_config"]]
    if cfg.get("local_mode"):
        main_args += ["--local-mode"]
    parser = argparse.ArgumentParser()
