...
def generate_gbm_prices(n_days: int, mu: float = 0.0002, sigma: float = 0.01, s0: float = 100.0) -> pd.DataFrame:
    """
    ...
    highs = np.maximum(opens, closes) * np.exp(sigma * np.abs(np.random.normal(0, 0.5, n_days)))
    
    # Low is the min of open and close minus a random amount
    lows = np.minimum(opens, closes) * np.exp(-sigma * np.abs(np.random.normal(0, 0.5, n_days)))
