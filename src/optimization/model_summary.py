...
class ModelSummarizer:
        ...
        for name, module in self.model.named_modules():
            ...
            if is_leaf:
                info.append({
                    ...
                    "requires_grad": all(p.requires_grad for p in module.parameters()) if layer_params > 0 else None
                })
    ...
    def get_summary(self, detailed: bool = True) -> str:
        ...
        lines.append(f"  - Activations (est.): {self.memory_estimate['activations']:.2f} MB")
        ...
        if detailed:
            lines.append(f"\nLayer details:")
            lines.append(f"{'-' * 80}")
            lines.append(f"{'Layer Name':<40} {'Type':<15} {'Params':>10} {'Trainable':<10}")
            lines.append(f"{'-' * 80}")
            for layer in self.layer_info:
                trainable = "Yes" if layer["requires_grad"] else "No" if layer["requires_grad"] is not None else "N/A"
                lines.append(f"{layer['name']:<40} {layer['type']:<15} {layer['parameters']:>10,} {trainable:<10}")
        
        return "\n".join(lines)

    def visualize_model_complexity(self, save_path: Optional[str] = None):
...
def optimal_gpu_config(
    ...
    if best_gpu:
        ...
        recommendations["gpu_memory_free_mb"] = best_gpu.get("memory_free", best_gpu["total_memory"])
        ...
        if total_memory_mb > available_memory:
            ...
            mixed_precision_savings = param_memory_mb * 0.5 + grad_memory_mb * 0.5  # Roughly half for fp16
            ...
            if new_total_memory > available_memory:
                ...
                recommendations["reason"] = "Using mixed precision due to memory constraints"
        else:
