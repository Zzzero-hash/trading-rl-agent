...
class SystemHealthMonitor:
            ...
            try:
                ...
                if self.alert_manager:
                    self.alert_manager.create_alert(
                        ...
                        message=f"Error in health monitoring loop: {str(e)}",
        ...
        for check_name, check_func in self.custom_health_checks.items():
            try:
                ...
                result = HealthCheckResult(
                    ...
                    status=HealthStatus.UNKNOWN,
                    message=f"Custom health check '{check_name}' failed: {str(e)}",
                    timestamp=time.time(),
                    error=str(e)
                )
                results.append(result)
    ...
    def _check_system_resources(self) -> HealthCheckResult:
        ...
        message = f"CPU: {metrics.cpu_percent:.1f}%, Memory: {metrics.memory_percent:.1f}%, Disk: {metrics.disk_percent:.1f}%"
    ...
    def generate_health_report(self, output_path: Optional[str] = None) -> str:
        ...
        report_lines = [
            ...
            f"Generated: {datetime.fromtimestamp(dashboard_data['timestamp'])}",
        ...
        for check in dashboard_data['recent_health_checks'][-10:]:  # Last 10 checks
            report_lines.append(
                f"[{check['status'].upper()}] {check['check_type']}: {check['message']}"
            )
