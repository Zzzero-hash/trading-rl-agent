...
class HealthDashboard:
    ...
    def create_health_visualization(self, save_path: Optional[str] = None) -> Figure:
    ...
    def _plot_system_metrics(self, ax, dashboard_data: Dict[str, Any]) -> None:
        """Plot system metrics."""
    ...
    def _plot_memory_usage(self, ax, dashboard_data: Dict[str, Any]) -> None:
        """Plot memory usage."""
        metrics = dashboard_data["current_metrics"]
        ...
        # Add value label
        ax.text(0, memory_percent + 2, f'{memory_percent:.1f}%', 
                ha='center', va='bottom', fontweight='bold')
    
    def _plot_disk_usage(self, ax, dashboard_data: Dict[str, Any]) -> None:
        """Plot disk usage."""
        metrics = dashboard_data["current_metrics"]
    ...
    def _plot_trading_metrics(self, ax, dashboard_data: Dict[str, Any]) -> None:
        ...
        values = [float(metric[1].replace('$', '').replace('%', '').replace('ms', '')) 
                 if metric[1] not in ['N/A', '0'] else 0 for metric in metrics_data]
        ...
        # Add value labels
        for i, (metric_name, metric_value) in enumerate(metrics_data):
            ax.text(values[i] + max(values) * 0.01, i, metric_value, 
                   va='center', fontweight='bold')
    
    def _plot_health_status(self, ax, dashboard_data: Dict[str, Any]) -> None:
        ...
        if sizes:
            ...
            ax.text(0.5, 0.5, 'No health checks\nperformed', ha='center', va='center',
                   transform=ax.transAxes, fontsize=12)
            ax.set_title('Health Status')
    
    def _plot_recent_checks(self, ax, dashboard_data: Dict[str, Any]) -> None:
        ...
        if not recent_checks:
            ax.text(0.5, 0.5, 'No recent health checks', ha='center', va='center',
                   transform=ax.transAxes, fontsize=12)
            ax.set_title('Recent Health Checks')
            return
        
        # Convert timestamps to datetime
        timestamps = [datetime.fromtimestamp(check['timestamp']) for check in recent_checks]
    ...
    def _create_html_content(self, dashboard_data: Dict[str, Any]) -> str:
        ...
        current_time = datetime.fromtimestamp(dashboard_data['timestamp'])
    ...
    def _create_health_checks_html(self, health_checks: List[Dict[str, Any]]) -> str:
        ...
        for check in health_checks[-10:]:  # Last 10 checks
            timestamp = datetime.fromtimestamp(check['timestamp'])
            html += f"""
            <div class="metric">
    ...
    def save_dashboard_data(self, filename: str = None) -> str:
