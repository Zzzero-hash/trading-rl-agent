...
class ParallelVaRCalculator:
    ...
    def _initialize_executors(self) -> None:
        ...
        if self.config.use_multiprocessing:
            self._process_executor = ProcessPoolExecutor(
                max_workers=self.config.n_processes
            )
    ...
    def parallel_monte_carlo_var(
        ...
        if self.config.use_multiprocessing and self._process_executor:
            chunk_results = self._execute_parallel_chunks_process(
                chunk_configs, returns_data, weights
            )
        else:
            chunk_results = self._execute_parallel_chunks_thread(
                chunk_configs, returns_data, weights
            )
    ...
    def _execute_parallel_chunks_process(
        ...
        args_list = [
            (config, returns_data, weights) for config in chunk_configs
        ]
        
        # Submit tasks
        futures = [
            self._process_executor.submit(self._calculate_chunk_var, *args)
        ...
        for future in as_completed(futures):
            try:
                ...
                self.logger.error(f"Error in parallel VaR calculation: {e}")
                raise
    ...
    def _execute_parallel_chunks_thread(
        ...
        # Submit tasks
        futures = [
            self._thread_executor.submit(self._calculate_chunk_var, *args)
            for args in args_list
        ]
    ...
    def parallel_stress_test(
        ...
        # Prepare arguments for parallel execution
        args_list = [
            (var_config, returns_data, weights, scenario) for scenario in scenarios
        ]
        
        # Execute parallel stress tests
        if self.config.use_multiprocessing and self._process_executor:
            futures = [
                self._process_executor.submit(self._calculate_stress_scenario, *args)
                for args in args_list
            ]
        else:
            futures = [
                self._thread_executor.submit(self._calculate_stress_scenario, *args)
                for args in args_list
            ]
        ...
        for i, future in enumerate(as_completed(futures)):
            try:
                ...
                self.logger.error(f"Error in stress test for scenario {scenarios[i]}: {e}")
                raise
    ...
    def parallel_method_comparison(
        ...
        # Prepare arguments for parallel execution
        args_list = [
            (var_config, returns_data, weights, method) for method in methods
        ]
        
        # Execute parallel method comparisons
        if self.config.use_multiprocessing and self._process_executor:
            futures = [
                self._process_executor.submit(self._calculate_method_var, *args)
                for args in args_list
            ]
        else:
            futures = [
                self._thread_executor.submit(self._calculate_method_var, *args)
                for args in args_list
            ]
        ...
        for i, future in enumerate(as_completed(futures)):
            try:
                ...
                self.logger.error(f"Error in method comparison for {methods[i]}: {e}")
                raise
