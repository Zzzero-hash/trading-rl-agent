...
class MonteCarloVaRConfig:
    ...
    # Stress testing
    stress_scenarios: dict[str, dict[str, float]] | None = field(default=None)
...
class MonteCarloVaR:
    ...
    def __init__(self, config: MonteCarloVaRConfig):
        ...
        # Data storage
        self._returns_data: pd.DataFrame | None = None
    ...
    def update_data(self, returns_data: pd.DataFrame) -> None:
        ...
        if len(self._returns_data) < self.config.lookback_period:
            self.logger.warning(f"Insufficient data: {len(self._returns_data)} < {self.config.lookback_period}")
    ...
    def _calculate_covariance_matrix(self) -> None:
        ...
        try:
            ...
            self.logger.exception(f"Error calculating covariance matrix: {e}")
    ...
    def _calculate_correlation_matrix(self) -> None:
        ...
        try:
            ...
            self.logger.exception(f"Error calculating correlation matrix: {e}")
            self._correlation_matrix = pd.DataFrame()

    def historical_simulation_var(self, weights: dict[str, float], use_bootstrap: bool = True) -> VaRResult:
        """
    ...
    def _bootstrap_historical_var(self, returns: pd.Series) -> tuple[float, float]:
        ...
        """
        var_bootstrap: list[float] = []
        cvar_bootstrap: list[float] = []
        ...
        for _ in range(self.config.bootstrap_samples):
            ...
            var_bootstrap.append(float(np.percentile(bootstrap_returns, self.config.confidence_level * 100)))
            cvar_bootstrap.append(float(bootstrap_returns[bootstrap_returns <= var_bootstrap[-1]].mean()))

        # Return mean of bootstrap samples
        return float(np.mean(var_bootstrap)), float(np.mean(cvar_bootstrap))

    def parametric_var(self, weights: dict[str, float], distribution: str | None = None) -> VaRResult:
        """
    ...
    def _calculate_portfolio_statistics(self, weights: dict[str, float]) -> tuple[float, float]:
        ...
        # Portfolio mean
        portfolio_mean = sum(weights[asset] * self._returns_data[asset].mean() for asset in available_assets)
        ...
        return float(portfolio_mean), portfolio_std
    ...
    def _calculate_normal_cvar(self, mean: float, std: float) -> float:
        ...
        return float(mean - std * float(norm.pdf(norm.ppf(alpha))) / alpha)
    ...
    def _calculate_t_cvar(self, df: float, loc: float, scale: float) -> float:
        ...
        return float(loc - scale * float(stats.t.pdf(t_alpha, df)) * (df + t_alpha**2) / (df - 1) / alpha)

    def _calculate_laplace_cvar(self, mean: float, std: float) -> float:
        """Calculate CVaR for Laplace distribution."""
        alpha = self.config.confidence_level
        return float(mean - std * (1 + np.log(alpha)))

    def _calculate_logistic_cvar(self, mean: float, std: float) -> float:
        """Calculate CVaR for logistic distribution."""
        alpha = self.config.confidence_level
        return float(mean - std * (1 + np.log(alpha / (1 - alpha))))

    def monte_carlo_var(self, weights: dict[str, float], use_correlation: bool = True) -> VaRResult:
        """
    ...
    def _generate_correlated_returns(self) -> np.ndarray:
        ...
        return uncorrelated @ chol_matrix.T
    ...
    def _calculate_simulated_portfolio_returns(
        ...
        return simulated_returns[:, asset_indices] @ weight_vector

    def stress_test_var(self, weights: dict[str, float], scenario: str | None = None) -> dict[str, VaRResult]:
        """
        ...
        if self.config.stress_scenarios is None:
            raise ValueError("No stress scenarios configured")
        scenarios = [scenario] if scenario else list(self.config.stress_scenarios.keys())
    ...
    def backtest_var(self, weights: dict[str, float], method: str = "monte_carlo") -> dict[str, Any]:
        """
        ...
        for i in range(window_size, len(self._returns_data), step_size):
            ...
            if i < len(self._returns_data):
                actual_return = self._calculate_portfolio_returns(weights, self._returns_data.iloc[i : i + 1]).iloc[0]
    ...
    def _christoffersen_test(self, breaches: np.ndarray) -> tuple[float, float]:
        ...
        # Likelihood ratio test statistic
        if p01 == 0 or p11 == 0 or p in {0, 1}:
            return 0.0, 1.0

        lr = 2 * (
            n01 * np.log(p01 / p)
            + n11 * np.log(p11 / p)
            + n00 * np.log((1 - p01) / (1 - p))
            + n10 * np.log((1 - p11) / (1 - p))
        )
    ...
    def _calculate_portfolio_returns(self, weights: dict[str, float], data: pd.DataFrame | None = None) -> pd.Series:
