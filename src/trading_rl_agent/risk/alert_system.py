...
class RiskAlertSystem:
    ...
    def __init__(
        ...
        self.last_risk_check: float | None = None
    ...
    def _get_metric_value(self, metrics: RiskMetrics, metric_name: str) -> float | None:
    ...
    def _is_threshold_violated(self, value: float, threshold: AlertThreshold) -> bool:
        ...
        if threshold.threshold_type == "max":
            return value > threshold.threshold_value
        if threshold.threshold_type == "change_rate":
    ...
    def _should_trigger_circuit_breaker(self, rule: CircuitBreakerRule, metrics: RiskMetrics) -> bool:
        ...
        if rule.trigger_condition == "drawdown_exceeded":
            return metrics.current_drawdown > rule.threshold_value
        if rule.trigger_condition == "leverage_exceeded":
            return metrics.leverage > rule.threshold_value
        if rule.trigger_condition == "correlation_risk_exceeded":
...
Source: {alert.source}
Time: {datetime.fromtimestamp(alert.timestamp, tz=UTC)}
...
Please review and take appropriate action.
    ...
    def _get_current_escalation_level(self) -> str:
        ...
        critical_alerts = [
            a for a in self.alert_history if a.severity == AlertSeverity.CRITICAL and a.status == AlertStatus.ACTIVE
        ]
        error_alerts = [
            a for a in self.alert_history if a.severity == AlertSeverity.ERROR and a.status == AlertStatus.ACTIVE
        ]
    ...
    def generate_risk_report(
        ...
        filtered_alerts = [
            ...
            if start_time <= datetime.fromtimestamp(alert.timestamp, tz=UTC) <= end_time
        ]

        filtered_metrics = [metrics for metrics in self.risk_history if start_time <= metrics.timestamp <= end_time]
    ...
    def _get_escalation_summary(self, start_time: datetime, end_time: datetime) -> dict[str, Any]:
        ...
        filtered_escalations = [
            ...
            if start_time <= datetime.fromtimestamp(e["timestamp"], tz=UTC) <= end_time
        ]
    ...
    def get_risk_history(self, limit: int | None = None) -> list[RiskMetrics]:
