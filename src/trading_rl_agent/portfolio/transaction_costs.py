"""
Comprehensive transaction cost modeling for realistic backtesting.

This module provides a complete transaction cost modeling system that includes:
...
from trading_rl_agent.utils.logging import get_logger
...
class LinearImpactModel(MarketImpactModel):
    ...
    def calculate_impact(self, order_size: float, market_data: MarketData) -> float:
        ...
        return float(order_size * self.impact_rate * volume_ratio)
...
class SquareRootImpactModel(MarketImpactModel):
    ...
    def calculate_impact(self, order_size: float, market_data: MarketData) -> float:
        ...
        volume_ratio = order_size / market_data.avg_daily_volume if market_data.avg_daily_volume > 0 else 0
        return float(order_size * self.impact_rate * np.sqrt(volume_ratio))
...
class AdaptiveImpactModel(MarketImpactModel):
    ...
    def calculate_impact(self, order_size: float, market_data: MarketData) -> float:
        ...
        return float(order_size * self.base_impact_rate * np.sqrt(volume_ratio) * volatility_factor * liquidity_factor)
...
class VolumeBasedSlippageModel(SlippageModel):
    ...
    def calculate_slippage(self, order_size: float, market_data: MarketData, order_type: OrderType) -> float:
        ...
        return float(order_size * slippage_rate)
...
class TransactionCostModel:
    ...
    def simulate_execution(
        ...
        filled_quantity, partial_fills = self.partial_fill_model.simulate_fill(requested_quantity, market_data)
    ...
    def get_cost_summary(self) -> dict[str, Any]:
        ...
        total_costs = self.total_commission + self.total_slippage + self.total_market_impact + self.total_spread_cost
    ...
    def generate_optimization_recommendations(self) -> list[CostOptimizationRecommendation]:
        ...
        large_orders = [
            trade for trade in self.trade_history if trade["requested_quantity"] > 10000
        ]  # Arbitrary threshold
    ...
    def create_broker_model(cls, broker_type: BrokerType) -> "TransactionCostModel":
        ...
        if broker_type == BrokerType.PREMIUM:
            return cls(
        ...
        if broker_type == BrokerType.CRYPTO:
            return cls(
        ...
        # This should never be reached as all BrokerType values are handled above
        raise ValueError(f"Unsupported broker type: {broker_type}")


class TransactionCostAnalyzer:
    """
    Analyzer for transaction cost data and optimization recommendations.
    """

    def __init__(self, cost_model: TransactionCostModel):
        """Initialize the analyzer with a transaction cost model."""
    ...
    def analyze_cost_trends(self) -> dict[str, Any]:
        ...
        trades_by_period: dict[str, list[dict[str, Any]]] = {}
        for trade in self.cost_model.trade_history:
            period = trade["timestamp"].strftime("%Y-%m-%d")
        ...
        for period, trades in trades_by_period.items():
            total_cost = sum(trade["cost_breakdown"]["total_cost"] for trade in trades)
    ...
    def calculate_cost_efficiency_metrics(self) -> dict[str, float]:
        ...
        total_costs = sum(trade["cost_breakdown"]["total_cost"] for trade in self.cost_model.trade_history)
        total_value = sum(
            abs(trade["requested_quantity"] * trade["market_data"].mid_price) for trade in self.cost_model.trade_history
        )
...
- Average Delay per Trade: {summary["avg_delay_per_trade"]:.2f}s
...
Efficiency Metrics:
- Cost Efficiency Ratio: {efficiency.get("cost_efficiency_ratio", 0):.4f}
- Cost Percentage: {efficiency.get("cost_percentage", 0):.2f}%
