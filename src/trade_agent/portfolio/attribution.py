...
class FactorModel:
    """Implements factor model for return decomposition."""
    ...
    def __init__(self, config: AttributionConfig):
        ...
        self.r_squared: pd.Series | None = None
    ...
    def _calculate_loadings(self, returns: pd.DataFrame) -> None:
        ...
        if self.factors is None:
            raise ValueError("Factors must be set before calculating loadings")
...
class BrinsonAttributor:
    ...
    def calculate_attribution(
        ...
        Args:
            ...
            benchmark_returns: Benchmark returns by group
        ...
        common_index = (
            portfolio_weights.index.intersection(benchmark_weights.index)
    ...
    def calculate_sector_attribution(
        ...
        for date in returns_data.index:
            if date in portfolio_data.index and date in benchmark_data.index:
                ...
                benchmark_returns = returns  # Use returns as benchmark returns for simplicity
...
class PerformanceAttributor:
    ...
    def _calculate_brinson_attribution(
        ...
        for date in portfolio_weights.index:
            if date in benchmark_weights.index and date in asset_returns.index:
                try:
                    portfolio_weights_sector = (
                        portfolio_with_sector.loc[date].groupby(self.config.sector_column)["weight"].sum()
                    )
                    benchmark_weights_sector = (
                        benchmark_with_sector.loc[date].groupby(self.config.sector_column)["weight"].sum()
                    )
                    ...
                    attribution = self.brinson_attributor.calculate_attribution(
                        ...
                        benchmark_weights_sector,
                        returns_sector,
                        returns_sector,  # Use returns_sector as benchmark_returns
                    )
    ...
    def create_dashboard(self, portfolio_returns: pd.Series, benchmark_returns: pd.Series) -> Any:
