...
def dynamic_quantization(model: nn.Module, 
...
def static_quantization(model: nn.Module,
    ...
    if calibration_data is None:
        warnings.warn("Static quantization requires calibration data - using dynamic instead")
        return dynamic_quantization(model)
...
def compare_model_sizes(original_model: nn.Module, 
...
def quantize_for_deployment(model: nn.Module,
                           sample_input: torch.Tensor,
                           target_platform: str = 'cpu') -> Dict[str, Any]:
    """
