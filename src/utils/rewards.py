...
def risk_adjusted_reward(returns: Union[float, np.ndarray], 
                        volatility: float = None,
                        risk_penalty: float = 0.1) -> float:
    """
...
def drawdown_penalty_reward(current_return: float,
...
def sharpe_based_reward(returns: np.ndarray, 
...
def portfolio_diversification_reward(weights: np.ndarray, 
                                   correlation_matrix: Optional[np.ndarray] = None,
                                   diversification_bonus: float = 0.1) -> float:
    """
...
def transaction_cost_penalty(action: Union[int, np.ndarray],
...
def momentum_reward(price_changes: np.ndarray, 
...
def custom_trading_reward(portfolio_return: float,
    ...
    if weights is not None:
        diversification_bonus = portfolio_diversification_reward(
            weights, 
            diversification_bonus=kwargs.get('diversification_bonus', 0.02)
        )
...
class RewardFunction:
    ...
    def _custom_reward(self, prev_balance: float, current_balance: float, **kwargs) -> float:
