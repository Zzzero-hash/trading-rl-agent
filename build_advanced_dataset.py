...
class AdvancedDatasetBuilder:
    ...
    def generate_advanced_synthetic_data(self) -> pd.DataFrame:
        ...
        synthetic_combined = synthetic_combined.sort_values(['symbol', 'timestamp']).reset_index(drop=True)
        
        logger.info(f"Generated {len(synthetic_combined)} synthetic data points across {len(symbols)} symbols")
    ...
    def _add_regime_patterns(self, df: pd.DataFrame, regime: Dict) -> pd.DataFrame:
        ...
        if regime['name'] == 'bull_market':
            ...
            df['close'] = df['close'] * (1 + np.cumsum(np.random.normal(0.0001, 0.001, len(df))))
            
        elif regime['name'] == 'bear_market':
            # Add downward pressure and volatility clustering
            df['close'] = df['close'] * (1 - np.cumsum(np.random.normal(0.0001, 0.002, len(df))))
    ...
    def fetch_real_market_data(self) -> pd.DataFrame:
        ...
        # Fetch crypto data
        for crypto in tqdm(CRYPTO_PAIRS[:3], desc="Fetching crypto data"):  # Limit to top 3
            try:
        ...
        if real_datasets:
            ...
            real_combined = real_combined.sort_values(['symbol', 'timestamp']).reset_index(drop=True)
    ...
    def add_advanced_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if enhanced_datasets:
            ...
            logger.info(f"Added features to {len(result)} data points across {len(enhanced_datasets)} symbols")
            return result
        else:
            logger.error("No enhanced datasets were created")
            return df
    
    def _add_symbol_specific_features(self, df: pd.DataFrame, symbol: str) -> pd.DataFrame:
        """Add features specific to the asset class."""
        df = df.copy()
        
        # Asset class encoding
        if hasattr(df, 'asset_class'):
            asset_class = df['asset_class'].iloc[0] if 'asset_class' in df.columns else 'unknown'
        else:
    ...
    def _calculate_usd_strength_proxy(self, df: pd.DataFrame) -> np.ndarray:
        ...
        for i in range(1, len(returns)):
            usd_strength[i] = alpha * (-returns.iloc[i]) + (1 - alpha) * usd_strength[i-1]
    ...
    def _add_regime_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['vol_regime_high'] = (df['vol_20'] > df['vol_20'].rolling(100).quantile(0.8)).astype(int)
        df['vol_regime_low'] = (df['vol_20'] < df['vol_20'].rolling(100).quantile(0.2)).astype(int)
            ...
            for symbol in forex_symbols:
                try:
                    ...
                    if sentiment_data:
                        ...
                        df.loc[df['symbol'] == symbol, f'forex_sentiment_{symbol}'] = avg_sentiment
                    else:
    ...
    def generate_labels(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for symbol in df['symbol'].unique():
            ...
            for horizon in [1, 3, 5]:
                ...
                rolling_vol = symbol_data['close'].pct_change().rolling(vol_window).std()
                ...
                symbol_data[f'label_{horizon}d'] = np.select(conditions, choices, default=1)
    ...
    def validate_data_quality(self, df: pd.DataFrame) -> Dict:
        ...
        logger.info(f"Data validation completed: {len(validation_results['data_quality_issues'])} issues found")
...
def main():
    ...
    parser.add_argument('--start_date', default='2020-01-01', help='Start date for real data')
    parser.add_argument('--end_date', default='2024-12-31', help='End date for real data')
    parser.add_argument('--synthetic_days', type=int, default=500, help='Days of synthetic data per symbol')
