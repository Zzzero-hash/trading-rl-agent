...
async def run_streaming_example() -> None:
    ...
    simulator_thread = threading.Thread(target=run_data_simulator, args=(metrics_collector, stop_event), daemon=True)
...
Examples:
  ...
  python dashboard_example.py config
    ...
    parser.add_argument("mode", choices=["basic", "streaming", "config"], help="Example mode to run")
