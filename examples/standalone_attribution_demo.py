    ...
    def decompose_returns(self, returns: pd.DataFrame) -> Dict[str, pd.DataFrame]:
        ...
        for asset in returns.columns:
            systematic_returns[asset] = (
                self.factor_loadings.loc[asset] * self.factor_returns
            ).sum(axis=1)
    ...
    def analyze_factor_contributions(self, portfolio_returns: pd.Series, 
        ...
        for factor in factor_returns.columns:
            if factor in self.factor_returns.columns:
                ...
                portfolio_exposure = self.factor_loadings[factor].mean()
...
class PerformanceAttributor:
    ...
    def analyze_performance(self, portfolio_returns: pd.Series,
                          ...
                          benchmark_weights: pd.DataFrame,
                          sector_data: Optional[pd.DataFrame] = None) -> Dict[str, Any]:
...
def create_sample_data():
    ...
    for date in dates:
        benchmark_weights.loc[date] = market_caps_normalized
