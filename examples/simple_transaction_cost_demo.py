...
class MockNumpy:
    ...
    @staticmethod
    def choice(choices: list, size: int | None = None, p: list | None = None) -> list | Any:  # noqa: ARG004
        if size is None:
...
class MockPandas:
    @staticmethod
    def DataFrame(data: Any) -> "MockDataFrame":
        return MockDataFrame(data)

    @staticmethod
    def Series(data: Any, index: Any = None) -> "MockSeries":
...
class MockSeries:
    def __init__(self, data: Any, index: Any = None) -> None:
        self.data = data
...
class MockRolling:
    def __init__(self, data: Any, window: int) -> None:
        self.data = data
        self.window = window
    ...
    def mean(self) -> list[float]:
        ...
        for i in range(len(self.data)):
            if i < self.window - 1:
                ...
                result.append(float(sum(window_data) / len(window_data)))
        return result


class MockPivotTable:
    def __init__(self, data: Any, index: str, columns: str, values: str) -> None:
        self.data = data
        self.index = index
...
def demonstrate_broker_types() -> None:
...
def demonstrate_market_conditions() -> None:
...
def demonstrate_commission_structures() -> None:
