...
def create_custom_health_check(monitor: SystemHealthMonitor) -> None:
    ...
    def custom_database_check() -> HealthCheckResult:
...
def setup_alert_handlers(alert_manager: AlertManager) -> None:
    ...
    def critical_alert_handler(alert: Any) -> None:
        """Handle critical alerts."""
        print(f"🚨 CRITICAL ALERT: {alert.title}")
        print(f"   Message: {alert.message}")
        print(f"   Source: {alert.source}")
        print(f"   Time: {datetime.fromtimestamp(alert.timestamp, tz=UTC)}")
        print("-" * 50)

    def warning_alert_handler(alert: Any) -> None:
    ...
    def info_alert_handler(alert: Any) -> None:
        """Handle info alerts."""
        print(f"i INFO: {alert.title}")
        print(f"   Message: {alert.message}")
        print("-" * 20)
