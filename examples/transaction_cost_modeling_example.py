...
def demonstrate_slippage_models():
    ...
    for model_name, model in slippage_models:
        ...
        slippage_normal = model.calculate_slippage(
            trade_value, market_data_normal, OrderType.MARKET
        )
        
        # Test with wide spread
        slippage_wide = model.calculate_slippage(
            trade_value, market_data_wide, OrderType.MARKET
        )
...
def demonstrate_cost_optimization():
    ...
    for quantity, price, order_type, market_condition in trade_scenarios:
        ...
        print(f"    Cost: ${execution_result.total_cost:.2f} ({(execution_result.total_cost/(quantity*price)*100):.3f}%)")
