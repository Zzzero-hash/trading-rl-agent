...
class AlpacaDemo:
    ...
    def __init__(self, config_file: str | None = None, symbols: list[str] | None = None):
        """
        Initialize the demo.
    ...
    def setup_configuration(self) -> AlpacaConfig:
        ...
        try:
            ...
            logger.exception(f"Failed to setup configuration: {e}")
            raise

    def initialize_alpaca(self, config: AlpacaConfig) -> None:
        ...
        try:
            ...
            # Validate connection
            if not self.alpaca.validate_connection():  # type: ignore
                raise Exception("Failed to validate Alpaca connection")

            logger.info("Alpaca integration initialized successfully")

        except Exception as e:
            logger.exception(f"Failed to initialize Alpaca integration: {e}")
            self.alpaca = None
            raise

    def _ensure_alpaca_initialized(self) -> AlpacaIntegration:
        """Ensure Alpaca integration is initialized."""
        if self.alpaca is None:
            raise RuntimeError("Alpaca integration not initialized. Call initialize_alpaca() first.")
        # At this point, mypy knows self.alpaca is not None due to the above check
        return self.alpaca  # type: ignore

    def demo_account_info(self) -> None:
        """Demonstrate account information retrieval."""
        logger.info("\n" + "=" * 50)
        logger.info("DEMO: Account Information")
        logger.info("=" * 50)

        alpaca = self._ensure_alpaca_initialized()
    ...
    def demo_historical_data(self) -> None:
    ...
    def demo_real_time_quotes(self) -> None:
        ...
        try:
            ...
            quotes = alpaca.get_real_time_quotes(self.symbols)
            ...
            logger.exception(f"Failed to get real-time quotes: {e}")

    def demo_portfolio_monitoring(self) -> None:
        ...
        try:
            ...
            logger.exception(f"Failed to get portfolio information: {e}")

    def demo_paper_trading(self) -> None:
        ...
        if not alpaca.config.paper_trading:
            logger.warning("Paper trading is disabled. Skipping order execution demo.")
            return

        try:
            # Get current quotes for first symbol
            quotes = alpaca.get_real_time_quotes([self.symbols[0]])
            symbol = self.symbols[0]
            quote = quotes[symbol]

            logger.info(f"Current market for {symbol}:")
            ...
            buy_order_request = OrderRequest(
                ...
                time_in_force="day",
                client_order_id=f"demo_buy_order_{int(time.time())}",
            )

            logger.info(f"\nPlacing market buy order for 1 share of {symbol}")

            order_result = alpaca.place_order(buy_order_request)
            ...
            logger.info(f"  Filled Qty: {order_result['filled_qty']}")
            if order_result["filled_avg_price"]:
                logger.info(f"  Filled Avg Price: ${order_result['filled_avg_price']:.2f}")
            ...
            sell_order_result = alpaca.place_order(sell_order_request)
            ...
            logger.exception(f"Failed to execute paper trading demo: {e}")

    def data_stream_callback(self, data_type: str, data: Any) -> None:
        """Callback for real-time data stream."""
        self.data_updates += 1

        if data_type == "bar":
            logger.info(
                f"BAR UPDATE [{self.data_updates}]: {data.symbol} - Close: ${data.close:.2f}, Volume: {data.volume}"
            )
    ...
    def demo_real_time_streaming(self, duration: int = 30) -> None:
        ...
        try:
            ...
            # Add callback
            alpaca.add_data_callback(self.data_stream_callback)

            # Start streaming
            alpaca.start_data_stream(self.symbols)
            ...
            logger.exception(f"Failed to start data streaming: {e}")
            alpaca.stop_data_stream()

    def demo_order_history(self) -> None:
    ...
    def demo_asset_information(self) -> None:
        ...
        try:
            ...
            logger.exception(f"Failed to get asset information: {e}")
