...
def demonstrate_basic_var_calculations():
...
def demonstrate_parallel_processing():
    ...
    with ParallelVaRCalculator(parallel_config) as parallel_calc:
        ...
        parallel_result = parallel_calc.parallel_monte_carlo_var(
            var_config, returns_data, weights
        )
        ...
        sequential_config = MonteCarloVaRConfig(
            n_simulations=20000,
            confidence_level=0.05,
            use_parallel=False
        )
...
def demonstrate_method_comparison():
    ...
    for method_name, result in results.items():
        print(f"{method_name:<20} {result.var_value:<10.4f} {result.cvar_value:<10.4f} "
