...
class TestDataSanitizationSecurity:
    ...
    def test_html_encoding(self, mock_data_loader):
        ...
        for test_input in test_inputs:
            ...
            # Verify encoding
            assert "&lt;" in encoded or "&gt;" in encoded or "&amp;" in encoded or "&quot;" in encoded or "&#x27;" in encoded
    ...
    def test_file_extension_sanitization(self, mock_data_loader):
        ...
        for ext in invalid_extensions:
            with pytest.raises(ValueError):
                ...
                if ext in [".exe", ".bat", ".sh", ".py", ".js", ".html", ".php", ".asp", ".jsp", 
                          ".jar", ".war", ".ear", ".dll", ".so", ".dylib", ".bin", ".com", 
                          ".cmd", ".ps1", ".vbs", ".wsf", ".msi", ".pkg", ".deb", ".rpm", 
                          ".app", ".dmg", ".iso"]:
