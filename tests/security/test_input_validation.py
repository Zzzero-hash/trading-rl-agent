...
class TestInputValidationSecurity:
    ...
    def test_xss_prevention(self, mock_config):
        ...
        for malicious_script in malicious_scripts:
            with pytest.raises(ValueError):
                # Test XSS prevention
                if any(tag in malicious_script.lower() for tag in ["<script>", "javascript:", "onload=", "onerror=", "onclick=", "<img", "<svg"]):
    ...
    def test_config_validation(self, mock_config):
        ...
        for invalid_config in invalid_configs:
            with pytest.raises(ValueError):
                if "symbols" in invalid_config:
                    if not invalid_config["symbols"]:
                        raise ValueError("Symbols list cannot be empty")
                if "initial_balance" in invalid_config:
                    if invalid_config["initial_balance"] <= 0:
                        raise ValueError("Initial balance must be positive")
                if "max_position_size" in invalid_config:
                    if not (0 < invalid_config["max_position_size"] <= 1):
    ...
    def test_file_upload_validation(self, mock_config):
        ...
        invalid_extensions = [
            ".exe", ".bat", ".sh", ".py", ".js", ".html", ".php", ".asp", ".jsp",
            ".jar", ".war", ".ear", ".dll", ".so", ".dylib", ".bin", ".com",
        ]

        for ext in invalid_extensions:
            with pytest.raises(ValueError):
                if ext in [".exe", ".bat", ".sh", ".py", ".js", ".html", ".php", ".asp", ".jsp", ".jar", ".war", ".ear", ".dll", ".so", ".dylib", ".bin", ".com"]:
