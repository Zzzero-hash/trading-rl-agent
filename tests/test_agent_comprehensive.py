...
class TestAgentComprehensive:
    ...
    def test_agent_initialization(self, agent_type, agent_configs, mock_environment):
        ...
        try:
            ...
            # Test configuration
            assert (
                agent.config.learning_rate == agent_configs[agent_type]["learning_rate"]
            )
    ...
    def test_agent_training_pipeline(self, agent_type, agent_configs, mock_environment):
        ...
        try:
            ...
            if (
                ...
                assert (
                    len(found_metrics) > 0
                ), f"No expected metrics found in {training_info.keys()}"
    ...
    def test_agent_save_load(
        self, agent_type, agent_configs, mock_environment, tmp_path
    ):
        ...
        try:
            ...
            assert (
                save_path.exists()
                or Path(f"{save_path}.pkl").exists()
                or Path(f"{save_path}.pth").exists()
            )
            ...
            assert (
                action_diff < 2.0
            ), f"Loaded agent behavior differs significantly: {action_diff}"
    ...
    def test_agent_environment_training_loop(self, mock_environment, td3_config):
        ...
        try:
            ...
            for episode in range(total_episodes):
                ...
                print(
                    f"Episode {episode + 1}: {episode_steps} steps, reward: {episode_reward:.2f}"
                )
    ...
    def test_agent_performance_tracking(self, mock_environment, sac_config):
        ...
        try:
                ...
                for step in range(30):
                    ...
                    if (
                        ...
                        if "total_loss" in training_info:
                            performance_metrics["training_losses"].append(
                                training_info["total_loss"]
                            )
                        if "q_value" in training_info:
                            performance_metrics["q_values"].append(
                                training_info["q_value"]
                            )
            ...
            assert all(
                ...
                for r in performance_metrics["episode_rewards"]
            )
            assert all(
                isinstance(l, int) for l in performance_metrics["episode_lengths"]
            )

            print("✅ Performance tracking test passed")
            print(
                f"   Average episode reward: {np.mean(performance_metrics['episode_rewards']):.2f}"
            )
            print(
                f"   Average episode length: {np.mean(performance_metrics['episode_lengths']):.1f}"
            )
...
class TestAgentStressTests:
                    ...
                    if len(agent.replay_buffer) > config.batch_size:
                        ...
                        for key, value in training_info.items():
                            if isinstance(value, (int, float)):
                                assert not np.isnan(
                                    value
                                ), f"NaN in {key} at step {step}"
                                assert not np.isinf(
                                    value
                                ), f"Inf in {key} at step {step}"
    ...
    @pytest.mark.memory
    def test_agent_memory_efficiency(
        self, mock_environment, sac_config, memory_monitor
    ):
        ...
        try:
            ...
            print(
                f"✅ Memory efficiency test passed - memory increase: {memory_increase:.2f} MB"
            )
