...
def test_trader_env_initialization(sample_data):
    ...
    env = TraderEnv(
        [sample_data], window_size=10, initial_balance=5000, transaction_cost=0.002
    )
...
def test_trader_env_insufficient_data(tmp_path):
    ...
    with pytest.raises(
        ValueError, match="Not enough data for the specified window_size"
    ):
...
def test_trader_env_transaction_costs(env):
    ...
    price_change = (
        env.data.loc[env.current_step - 1, "close"]
        - env.data.loc[env.current_step - 2, "close"]
    )
    expected_balance = (
        balance_after_buy + price_change
    )  # profit/loss from price movement
