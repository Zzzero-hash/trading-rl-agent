...
class TestEnsembleAgent:
    ...
    def test_agent_initialization(self, ensemble_config):
        ...
        np.testing.assert_array_almost_equal(
            np.array(list(agent.weights.values())), 
            [0.5, 0.5], 
            decimal=5
        )
    ...
    def test_train_all_agents(self, ensemble_agent):
        ...
        initial_params = [p.clone() for p in ensemble_agent.agents['sac'].actor.parameters()]
    ...
    def test_diversity_calculation(self, ensemble_agent, sample_state):
        ...
        identical_actions = {'sac': np.array([0.5, 0.5, 0.5]), 'td3': np.array([0.5, 0.5, 0.5])}
        ...
        different_actions = {'sac': np.array([1.0, -1.0, 0.0]), 'td3': np.array([-1.0, 1.0, 0.5])}
    ...
    def test_performance_window_limit(self, ensemble_agent):
        ...
        assert ensemble_agent.performance_history['td3'][-1] == -(window_size + 49) * 0.1
