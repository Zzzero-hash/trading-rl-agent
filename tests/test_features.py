...
def test_generate_features_dimensions(n_rows):
    ...
    df = pd.DataFrame({'timestamp': dates, 'open': prices, 'high': prices, 'low': prices, 'close': prices, 'volume': np.ones(n_rows)})
...
def test_generate_features_no_nan():
    ...
    df = pd.DataFrame({'timestamp': dates, 'open': prices, 'high': prices, 'low': prices, 'close': prices, 'volume': np.ones(30)})
    df_feat = generate_features(df)
    core_cols = ['timestamp', 'open', 'high', 'low', 'close', 'volume', 'log_return'] + \
                [f'sma_{w}' for w in [5, 10, 20]] + ['rsi_14', 'vol_20', 'sentiment']
