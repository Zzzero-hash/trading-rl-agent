...
def test_add_hf_sentiment_merges_scores():
    ...
    assert out.loc[out["timestamp"] == pd.Timestamp("2021-01-01"), "hf_sentiment"].iloc[
        0
    ] == pytest.approx(0.8)
    assert out.loc[out["timestamp"] == pd.Timestamp("2021-01-03"), "hf_sentiment"].iloc[
        0
    ] == pytest.approx(0.0)
...
def test_integration_hf_and_twitter():
    ...
    assert {"hf_sentiment", "twitter_sentiment"}.issubset(combined.columns)
...
def test_integration_full_pipeline(monkeypatch):
    ...
    assert {"hf_sentiment", "twitter_sentiment", "news_sentiment"}.issubset(out.columns)
