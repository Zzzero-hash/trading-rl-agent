...
from src.trading_rl_agent.portfolio.attribution import (
    AttributionConfig, PerformanceAttributor, FactorModel, BrinsonAttributor
)
from src.trading_rl_agent.portfolio.transaction_costs import (
    TransactionCostModel, BrokerType, MarketCondition, OrderType, MarketData
)
...
class TestPortfolioManager:
    ...
    def test_trade_execution(self):
        ...
        with patch.object(self.portfolio_manager.transaction_cost_model.partial_fill_model, 'simulate_fill') as mock_fill:
    ...
    def test_portfolio_optimization(self):
        ...
        optimal_weights = self.portfolio_manager.optimize_portfolio(
            target_symbols, price_data, method="max_sharpe"
        )
...
class TestPortfolioOptimization:
    ...
    def test_min_variance_optimization(self):
        ...
        optimal_weights = self.portfolio_manager.optimize_portfolio(
            target_symbols, price_data, method="min_variance"
        )
    ...
    def test_risk_parity_optimization(self):
        ...
        optimal_weights = self.portfolio_manager.optimize_portfolio(
            target_symbols, price_data, method="risk_parity"
        )
...
class TestPortfolioRebalancing:
    ...
    def test_rebalancing_needs_detection(self):
        ...
        # Calculate drift
        drift = sum(abs(weights.get(asset, 0) - target_weights.get(asset, 0)) 
    ...
    def test_portfolio_optimization_workflow(self):
        ...
        if optimal_weights is not None:
            ...
            for symbol, target_weight in optimal_weights.items():
                ...
                current_value = self.portfolio_manager.positions.get(symbol, Position(
                    symbol=symbol, quantity=0, entry_price=0, current_price=0, timestamp=datetime.now()
                )).market_value
...
class TestPortfolioPerformanceBenchmarks:
    ...
    def test_attribution_analysis_performance(self):
        ...
        asset_returns = pd.DataFrame({
            f'ASSET_{i}': np.random.normal(0.001, 0.02, 252)
            for i in range(20)
        }, index=dates)
...
class TestPortfolioErrorHandling:
    ...
    def test_optimization_with_insufficient_data(self):
        ...
        optimal_weights = self.portfolio_manager.optimize_portfolio(
            ['AAPL'], price_data, method="max_sharpe"
        )
