...
class TestTradeCLICommands:
    ...
    def test_status_command(self, mock_status):
        ...
        result = self.runner.invoke(main_app, ["trade", "status", "--session-id", "test_session", "--detailed"])
...
class TestCLIIntegration:
    ...
    def test_end_to_end_workflow(self, mock_backtest, mock_train, mock_process, mock_download):
        ...
        result = self.runner.invoke(main_app, ["data", "process", "--output-dir", self.temp_dir])
        assert result.exit_code == 0
        mock_process.assert_called_once()
