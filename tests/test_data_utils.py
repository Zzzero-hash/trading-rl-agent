...
class TestDataManager:
    ...
    def find_available_datasets(self, pattern: str = "*training_data*.csv") -> List[str]:
    ...
    def get_or_create_test_dataset(self, required_columns: Optional[List[str]] = None) -> str:
        ...
        if available_files:
            ...
            if required_columns:
                try:
                    ...
                    if missing_cols:
                        logger.warning(f"Dataset missing columns {missing_cols}, generating synthetic data")
                        return self._create_temp_dataset(required_columns)
                except Exception as e:
                    logger.warning(f"Error reading {dataset_path}: {e}, generating synthetic data")
                    return self._create_temp_dataset(required_columns)
