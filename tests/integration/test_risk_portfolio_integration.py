...
from src.trading_rl_agent.risk.manager import RiskManager, RiskLimits
from src.trading_rl_agent.risk.alert_system import RiskAlertSystem, RiskAlertConfig
from src.trading_rl_agent.portfolio.manager import PortfolioManager, PortfolioConfig
from src.trading_rl_agent.portfolio.attribution import PerformanceAttributor
...
class TestRealTimeRiskPortfolioIntegration:
            ...
            for symbol, target_weight in target_weights.items():
                ...
                if target_weight < current_weight:
                    ...
                    if reduction_value > 0:
                        reduction_quantity = int(reduction_value / self.portfolio_manager.positions[symbol].current_price)
                        if reduction_quantity > 0:
                            self.portfolio_manager.execute_trade(
                                symbol, -reduction_quantity, 
                                self.portfolio_manager.positions[symbol].current_price, "long"
                            )
    ...
    def test_risk_aware_trade_execution(self):
        ...
        potential_weights = {
            "AAPL": (self.portfolio_manager.positions["AAPL"].market_value + large_trade_quantity * large_trade_price) / potential_portfolio_value
        }
        ...
        if potential_var <= self.risk_limits.max_portfolio_var:
            ...
            max_quantity = int((self.risk_limits.max_portfolio_var - pre_trade_var) * potential_portfolio_value / large_trade_price)
            if max_quantity > 0:
...
class TestEndToEndTradingScenarios:
    ...
    def test_complete_trading_workflow(self):
        ...
        target_symbols = ['AAPL', 'GOOGL', 'MSFT']
        optimal_weights = self.portfolio_manager.optimize_portfolio(
            target_symbols, {}, method="max_sharpe"
        )
        ...
        if optimal_weights is not None:
            ...
            if abs(current_var - initial_var) > 0.01:  # 1% change threshold
                ...
                new_optimal_weights = self.portfolio_manager.optimize_portfolio(
                    target_symbols, {}, method="max_sharpe"
                )
