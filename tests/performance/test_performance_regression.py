...
class PerformanceBaseline:
    ...
    mean_throughput: float | None = None
    std_throughput: float | None = None
    mean_success_rate: float | None = None
    std_success_rate: float | None = None
    sample_count: int = 0
...
class PerformanceRegressionDetector:
        ...
        if self.baselines_file.exists():
            try:
                with open(self.baselines_file) as f:
                    data = json.load(f)
                    return {test_name: PerformanceBaseline(**baseline) for test_name, baseline in data.items()}
            except Exception as e:
    ...
    def get_performance_trends(self, test_name: str, days: int = 30) -> dict:
        ...
        test_metrics = [m for m in self.metrics if m.test_name == test_name and m.timestamp >= cutoff_date]
