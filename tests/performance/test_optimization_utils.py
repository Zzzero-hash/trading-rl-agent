...
def test_optimal_gpu_config_no_gpu(monkeypatch):
    monkeypatch.setattr(
        mu, "detect_gpus", lambda: {"available": False, "count": 0, "devices": []}
    )
...
def test_optimal_gpu_config_memory_constraint(monkeypatch):
    fake_info = {
        ...
        "count": 1,
        "devices": [
            {"index": 0, "name": "GPU", "total_memory": 200, "memory_free": 50}
        ],
    }
    monkeypatch.setattr(mu, "detect_gpus", lambda: fake_info)
    cfg = mu.optimal_gpu_config(
        model_params=10_000_000, batch_size=64, sequence_length=100, feature_dim=30
    )
