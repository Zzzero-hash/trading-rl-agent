...
from src.agents.sac_agent import Actor, Critic, QNetwork, ReplayBuffer, SACAgent
...
class TestSACAgent:
    ...
    def sac_agent(self, agent_config):
        ...
        return SACAgent(state_dim=10, action_dim=1, config=agent_config, device="cpu")
    ...
    def test_save_and_load(self, sac_agent):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            new_agent = SACAgent(
                state_dim=10, action_dim=1, config=sac_agent.config, device="cpu"
            )
...
class TestSACIntegration:
    ...
    def test_training_loop(self):
        ...
        agent = SACAgent(
            state_dim=5,
            action_dim=1,
            config={"batch_size": 16, "buffer_capacity": 100, "learning_rate": 1e-3},
            device="cpu",
        )
    ...
    def test_deterministic_vs_stochastic_actions(self):
        ...
        stochastic_actions = [
            agent.select_action(state, evaluate=False) for _ in range(10)
        ]

        # Get multiple deterministic actions
        deterministic_actions = [
            agent.select_action(state, evaluate=True) for _ in range(10)
        ]
