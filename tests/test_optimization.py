...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA not available for profiling")
...
class TestProfilingEdgeCases:
    ...
    def test_profile_model_inference_various_shapes(self):
        ...
        for batch_size in [1, 8]:
            for seq_len in [5, 20]:
                perf = profile_model_inference(model, batch_size=batch_size, sequence_length=seq_len, num_features=5, num_warmup=1, num_runs=1)
    ...
    def test_profile_model_inference_on_cpu(self):
        ...
        perf = profile_model_inference(model, batch_size=2, sequence_length=4, num_features=3, num_warmup=1, num_runs=1)
...
class TestHyperparamOptimization:
    ...
    def test_hyperparameter_search_space(self):
        ...
        from src.optimization.cnn_lstm_optimization import _get_default_cnn_lstm_search_space
...
class TestHyperparamOptimizationEdgeCases:
    ...
    def test_run_hyperparameter_optimization_invalid_train_fn(self):
        ...
        try:
            run_hyperparameter_optimization(bad_train_fn, config_space, num_samples=1, max_epochs_per_trial=1)
    ...
    def test_run_hyperparameter_optimization_minimal(self):
        ...
        analysis = run_hyperparameter_optimization(dummy_train_fn, config_space, num_samples=1, max_epochs_per_trial=1)
        assert analysis is not None
