...
class TestDataPreprocessingComprehensive:
    ...
    def test_feature_generation_comprehensive(self, sample_market_data):
        ...
        try:
            ...
            expected_features = [
                'log_returns', 'sma_10', 'sma_20', 'rsi', 'volatility',
            ...
            print(f"✅ Feature generation test passed - {len(found_features)} features generated")
    ...
    def test_technical_indicators_comprehensive(self, sample_market_data):
        ...
        try:
            from src.data.features import (
                compute_log_returns, compute_simple_moving_average, 
    ...
    def test_advanced_technical_indicators(self, sample_market_data):
        ...
        try:
            from src.data.features import (
                compute_ema, compute_macd, compute_bollinger_bands,
    ...
    def test_data_normalization(self, sample_market_data):
        ...
        try:
            ...
            if len(numeric_columns) > 0:
                ...
                normalized_data = scaler.fit_transform(features_df[numeric_columns].fillna(0))
                
                assert normalized_data.shape == (len(features_df), len(numeric_columns))
                assert np.all(normalized_data >= 0), "Min-max normalized values should be >= 0"
                assert np.all(normalized_data <= 1), "Min-max normalized values should be <= 1"
                
                # Test standard normalization
                from sklearn.preprocessing import StandardScaler
                std_scaler = StandardScaler()
                
                standardized_data = std_scaler.fit_transform(features_df[numeric_columns].fillna(0))
                
                assert standardized_data.shape == (len(features_df), len(numeric_columns))
                
                # Check that mean is approximately 0 and std is approximately 1
                means = np.mean(standardized_data, axis=0)
                stds = np.std(standardized_data, axis=0)
                
                assert np.allclose(means, 0, atol=1e-10), "Standardized data should have mean ≈ 0"
                assert np.allclose(stds, 1, atol=1e-10), "Standardized data should have std ≈ 1"
    ...
    def test_feature_engineering_pipeline(self, sample_market_data):
        ...
        try:
            ...
            for i, config in enumerate(configs):
                ...
                print(f"✅ Pipeline test {i+1} passed - {len(numeric_columns)} features")
    ...
    def test_time_series_features(self, sample_market_data):
        ...
        try:
            ...
            if 'timestamp' in sample_market_data.columns:
                sample_market_data['timestamp'] = pd.to_datetime(sample_market_data['timestamp'])
            
            features_df = generate_features(sample_market_data.copy())
            
            # Check for time-based features
            time_features = [col for col in features_df.columns 
                ...
                if len(values) > 0:
                    ...
                    if original_col in features_df.columns:
                        ...
                        if len(original_values) > 10 and len(values) > 10:
                            ...
                            assert not np.array_equal(values.iloc[:min(len(values), len(original_values))], 
    ...
    def test_preprocessing_performance(self, large_dataset, memory_monitor):
        ...
        try:
            ...
            print(f"✅ Performance test passed: {processing_time:.2f}s, {memory_increase:.2f}MB")
