...
class AdvancedDatasetBuilder:
    ...
    def _get_default_config(self) -> Dict:
        ...
        return {
            "symbols": {
                "stocks": ["AAPL", "GOOGL", "MSFT", "AMZN", "TSLA", "META", "NVDA", "JPM", "BAC", "XOM"],
                "forex": ["EURUSD=X", "GBPUSD=X", "USDJPY=X", "USDCHF=X", "USDCAD=X", "AUDUSD=X", "NZDUSD=X"],
        ...
        with tqdm(total=len(all_symbols), desc="Market data") as pbar:
            for symbol, category in all_symbols:
                try:
                    ...
                    # Add metadata
                    df['timestamp'] = df.index.tz_localize(None) if df.index.tz else df.index
    ...
    def apply_feature_engineering(self, datasets: List[pd.DataFrame]) -> pd.DataFrame:
        ...
        with tqdm(total=len(datasets), desc="Feature engineering") as pbar:
            for dataset in datasets:
                ...
                symbol = dataset['symbol'].iloc[0] if 'symbol' in dataset.columns else 'UNKNOWN'
                ...
                try:
                    ...
                    enhanced_data['price_change_pct'] = ((enhanced_data['close'] - enhanced_data['open']) / enhanced_data['open']) * 100
                    enhanced_data['high_low_pct'] = ((enhanced_data['high'] - enhanced_data['low']) / enhanced_data['low']) * 100
                    enhanced_data['body_size'] = abs(enhanced_data['close'] - enhanced_data['open'])
                    enhanced_data['upper_shadow'] = enhanced_data['high'] - np.maximum(enhanced_data['open'], enhanced_data['close'])
                    enhanced_data['lower_shadow'] = np.minimum(enhanced_data['open'], enhanced_data['close']) - enhanced_data['low']
                    
                    # Add volume features
                    enhanced_data['volume_ma_20'] = enhanced_data['volume'].rolling(20).mean()
                    enhanced_data['volume_ratio'] = enhanced_data['volume'] / enhanced_data['volume_ma_20']
                    enhanced_data['volume_change'] = enhanced_data['volume'].pct_change()
                    
                    enhanced_datasets.append(enhanced_data)
                    pbar.set_postfix({"symbol": symbol, "features": len(enhanced_data.columns)})
    ...
    def save_datasets(self, df: pd.DataFrame, training_df: pd.DataFrame) -> Dict[str, str]:
        ...
        if not self.config["output"]["advanced_dataset_path"]:
            self.config["output"]["advanced_dataset_path"] = f"data/advanced_trading_dataset_{timestamp}.csv"
        if not self.config["output"]["metadata_path"]:
            self.config["output"]["metadata_path"] = f"data/dataset_metadata_{timestamp}.json"
        ...
        metadata = {
            ...
            "data_completeness": float(100 - (df.isnull().sum().sum() / (len(df) * len(df.columns)) * 100)),
            "config": self.config,
    ...
    def build_dataset(self) -> Dict[str, str]:
        ...
        print(f"📊 Final dataset: {final_dataset.shape[0]:,} records, {final_dataset.shape[1]} features")
        print(f"🎯 Training dataset: {training_dataset.shape[0]:,} records, {training_dataset.shape[1]-1} features")
...
def main():
    ...
    try:
        ...
        try:
            ...
            env = TraderEnv([file_paths["training_data"]], window_size=10, initial_balance=10000)
            obs, info = env.reset()
